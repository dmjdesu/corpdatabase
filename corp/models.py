...
class TagSubcategory(models.Model):
    category = models.ForeignKey(
        TagCategory, on_delete=models.CASCADE, related_name="subcategories"
    )
    name = models.CharField(max_length=100)
    # 他のフィールドも必要に応じて追加
...
class TagSmallcategory(models.Model):
    subcategory = models.ForeignKey(
        TagSubcategory, on_delete=models.CASCADE, related_name="smallcategories"
    )
    name = models.CharField(max_length=100)
    # 他のフィールドも必要に応じて追加
...
class Company(models.Model):
    ...
    origin_industry = models.ForeignKey(
        OriginIndustry,
        on_delete=models.CASCADE,
        related_name="companies",
    ...
    revenue = models.DecimalField(
        max_digits=15, decimal_places=2, blank=True, null=True, verbose_name="収益"
    )
    ...
    postal_code = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="郵便番号"
    )
...
class Contact(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="contacts", verbose_name="企業"
    )
    first_name = models.CharField(max_length=100, verbose_name="名")
    last_name = models.CharField(max_length=100, verbose_name="姓")
    position = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="役職"
    )
    email = models.EmailField(blank=True, null=True, verbose_name="メールアドレス")
    phone_number = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="電話番号"
    ...
    mobile_number = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="携帯番号"
    )
...
class Note(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="notes", verbose_name="企業"
    )
...
class City(models.Model):
    ...
    name = models.CharField(
        max_length=50, verbose_name="市区町村名（漢字）", null=True, blank=True
    )
    name_kana = models.CharField(
        max_length=50, verbose_name="市区町村名（カナ）", null=True, blank=True
    )
