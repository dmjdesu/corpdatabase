...
from corp.models import OriginIndustryCategory, OriginIndustry
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        with open(file_path, newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                # 先頭にあるドットを削除
                row = {key.lstrip('\ufeff'): value.lstrip('.') for key, value in row.items()}
                ...
                industry, created = OriginIndustry.objects.get_or_create(
                    name=industry_name,
                    defaults={'description': industry_description, 'category': category}
                )
                if not created:
